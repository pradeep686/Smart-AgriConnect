...
const DiscussionForum = () => {
  ...
  // Mock current user - replace with your actual authentication
  const [currentUser] = useState({
    id: "user123", // This should match the user ID from your backend
    name: "Current User"
  });
  ...
  const handleEditDiscussion = (discussion) => {
    // Only allow edit if the discussion belongs to current user
    if (discussion.user === currentUser.id || discussion.user._id === currentUser.id) {
  ...
  // Check if current user owns the discussion
  const isOwner = (discussion) => {
    return discussion.user === currentUser.id || discussion.user._id === currentUser.id;
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setView("my")}
                className={`px-4 py-2 rounded-full ${view === "my" ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'}`}
              >
                My Discussions
              </button>
            </div>
