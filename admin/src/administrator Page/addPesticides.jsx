...
export default function AddPesticides() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/pesticides", {
        method: "POST",
        body: data,
      });
  ...
  return (
    <div >
      <div className="flex-1 max-w-3xl  mx-auto bg-white shadow-md rounded-lg p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Add New Pesticide</h2>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
          <input type="text" name="name" placeholder="Pesticide Name" value={formData.name} onChange={handleChange} className="p-2 border rounded bg-white col-span-2 text-sm" />
          ...
          <textarea name="composition" placeholder="Composition" value={formData.composition} onChange={handleChange} className="p-2 border rounded bg-white col-span-2 text-sm" rows={2} />
          ...
          <div className="flex justify-center mt-4 col-span-2">
            <button
              type="submit"
              className="!bg-green-500 text-white font-semibold px-6 !py-2 rounded transition-transform duration-200 hover:bg-green-600 hover:scale-101 active:scale-95 text-sm"
            >
              Add Pesticide
            </button>
          </div>
